...
int buildSymbolTable(struct astNode **nodePointer,
  ...
  if (node->nodeType == symbolTable) {
    ...
    if (node->nodeType == program) {
      ...
      nextSymbolTable = createAstNode(node->fileName, -1, -1,
          symbolTable, (union astNodeValue){}, 0, flag_null, 2,
          createAstNode(package->fileName, package->line, package->column, package->nodeType, package->value, 0, package->flags, 0),
          previousSymbolTable);
    } else if (node->nodeType == definitions) {
      nextSymbolTable =
          createAstNode(node->fileName, -1, -1, symbolTable, (union astNodeValue){}, 0, flag_null, 0);
      for (int i = 0; i < node->numChildren; i++) {
        struct astNode *child = node->children[i];
        addAstNode(&nextSymbolTable,
                   createAstNode(child->fileName, child->line, child->column, child->nodeType, child->value, child->type,
                                 child->flags, 0));
      }
